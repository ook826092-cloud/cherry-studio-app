...
const logger = loggerService.withContext('PaintingTool');
...
export function paintingModelOutput(output: PaintingResult): { type: 'text'; value: string } {
  if (isPaintingError(output)) return { type: 'text', value: output.error };
  if (output.length === 0) {
...
export async function resolveConfiguredPaintingModel(
  ...
  if (!isUniqueModelId(uniqueModelId)) return null;
...
function throwIfAborted(signal: AbortSignal | undefined): void {
  ...
  throw Object.assign(new Error('Image generation aborted'), { name: 'AbortError' });
}
...
export async function generateImageFromPrompt(
  ...
  if (!resolvedModel) return { error: PAINTING_MODEL_NOT_CONFIGURED_NOTE };
  ...
  try {
    ...
    return createdFiles.map((file) => ({ id: file.id, name: file.filename }));
  } catch (error) {

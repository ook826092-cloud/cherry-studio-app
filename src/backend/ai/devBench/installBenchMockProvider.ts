 ...
 * C3 已停止为新数据库种入布局基准数据；这里暂时只兼容仍保留旧 provider 行的开发数据库，
 * 等 Agent Session 原生基准替代旧工具后一起移除。
 *
 ...
 * 2. 选择通道因此退化成一行数据：旧基准 provider 的行 id。
 *    `buildOpenAICompatibleConfig` 会把 `provider.id` 原样写进 `providerSettings.name`，所以
 *    这个 id 就是这里的哨兵，不依赖 baseUrl 的格式化行为（后者会被 `formatBaseURL` 追加
 *    API 版本）。
 *
...
const LAYOUT_BENCH_PROVIDER_ID = 'layout-bench-mock';

...
describe('wrapToolsWithExecutionHooks', () => {
  ...
  it('fires start before execute and end after with the tool result', async () => {
    const order: string[] = [];
    const onToolExecutionStart = jest.fn<void, [ToolExecutionStartEvent]>(() => {
      order.push('start');
    });
    const onToolExecutionEnd = jest.fn<void, [ToolExecutionEndEvent]>(() => {
      order.push('end');
    });

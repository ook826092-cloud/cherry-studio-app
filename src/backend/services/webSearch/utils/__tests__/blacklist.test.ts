...
function response(
  results: (Omit<WebSearchResult, 'sourceInput'> & { sourceInput?: string })[],
): WebSearchResponse {
  return {
    query: 'hello',
    providerId: 'tavily',
...
describe('filterWebSearchResponseWithBlacklist', () => {
  it('filters results by regex and match-pattern blacklist rules', () => {
    const webSearchResponse = response([
      ...
      { title: 'Blocked by regex', content: 'blocked', url: 'https://evil.example/path' },
      { title: 'Allowed', content: 'ok', url: 'https://allowed.example/post' },
    ]);
  ...
  it('matches regex blacklist patterns against the full URL', () => {
    const webSearchResponse = response([
      ...
      { title: 'Allowed other path', content: 'ok', url: 'https://evil.example/other-path' },
    ]);

    const filtered = filterWebSearchResponseWithBlacklist(webSearchResponse, [
      '/evil\\.example\\/malicious-path$/',
    ]);
  ...
  it('ignores invalid patterns and preserves malformed result urls', () => {
    const webSearchResponse = response([
      { title: 'Malformed URL', content: 'kept', url: 'not-a-valid-url' },
    ]);
  ...
  it('matches wildcard host and path patterns with linear matching', () => {
    const webSearchResponse = response([
      ...
      { title: 'Allowed different path', content: 'ok', url: 'https://example.com/blog/post' },
    ]);

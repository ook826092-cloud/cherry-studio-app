 ...
 * Kept distinct from a plain null / `OAuthServiceError` so the UI can surface
 * "please retry" instead of sending the user through a full browser OAuth round
 * for what is really a momentary blip.
 */
 ...
 * payloads, and the logger serializes errors verbatim. Pass the result to
 * `logger.error(msg, describeOAuthError(err))` instead of the raw error.
 */
export function describeOAuthError(error: unknown): {
  name: string;
  message: string;
  status?: number;
} {
  ...
  for (let depth = 0; depth < 5 && current instanceof Error; depth++) {
    ...
    if (typeof status === 'number') {
      return { message: current.message, name: current.name, status };
    }

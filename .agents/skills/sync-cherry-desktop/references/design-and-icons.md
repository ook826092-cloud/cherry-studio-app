...
- The host layer (`src/frontend/styles/global.css`) carries only what the token package cannot: the React Native `--font-mono` override, the accessibility-step-0 type-scale snapshot, the emoji line-height utilities, and the HeroUI bridge. Product roles belong in the token layer; page backgrounds use the canonical `background`, while components earn `card`, `secondary`, or `popover` from their semantic role instead of defining feature-local surfaces.

...
export function createSignatureGenerator(
  clientSecret: string,
  clientId = CLIENT_ID,
): (options: SignatureOptions) => SignatureHeaders {
  const client = new SignatureClient(clientId, `${clientSecret}.${CLIENT_SECRET_SUFFIX}`);
  return (options) => client.generateSignature(options);
}

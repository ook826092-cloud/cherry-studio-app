...
class AihubmixFluxTransport implements ImageGenerationTransport {
  ...
  async submit(input: ImageGenerationSubmitInput): Promise<{ taskId: string }> {
    ...
    else if (typeof bag.seed === 'string' && /^-?\d+$/.test(bag.seed.trim()))
      inputBody.seed = Number(bag.seed.trim());

    const safety = bag.safetyTolerance ?? bag.safety_tolerance;
    if (typeof safety === 'number') inputBody.safety_tolerance = safety;
    else if (typeof safety === 'string' && /^\d+$/.test(safety))
      inputBody.safety_tolerance = Number(safety);
  ...
  async poll(taskId: string, options: { signal?: AbortSignal }): Promise<string[]> {
    ...
    while (true) {
      ...
      if (status === 'Error' || status === 'Request Moderated' || status === 'Content Moderated') {
        throw createPaintingGenerateError('REMOTE_ERROR', {
          message: json?.detail || String(status),
        });
      }
...
export function createAihubmixFluxTransport(
  settings: AihubmixFluxTransportSettings,
): AihubmixFluxTransport {

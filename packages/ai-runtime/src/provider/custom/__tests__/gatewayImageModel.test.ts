...
import {
  createGatewayGeminiImageModel,
  isGatewayGeminiImageModel,
} from '../gateway/gatewayImageModel';
...
describe('createGatewayGeminiImageModel', () => {
  ...
  it('maps aspectRatio onto google.imageConfig', async () => {
    ...
    expect(doGenerate.mock.calls[0][0].providerOptions.google.imageConfig).toEqual({
      aspectRatio: '16:9',
    });
  });
  ...
  it('preserves explicit Google imageSize over the inherited Gateway resolution', async () => {
    ...
    expect(sent.providerOptions.google.imageConfig).toEqual({
      imageSize: '2K',
      aspectRatio: '16:9',
    });
    expect(sent.providerOptions.google.responseModalities).toEqual(['IMAGE']);
  ...
  it('rejects mask-based editing instead of silently ignoring it', async () => {
    ...
    await expect(
      model.doGenerate(
        callOptions({ mask: { type: 'file', mediaType: 'image/png', data: 'MASK' } }),
      ),
  ...
  it('forwards input images as file parts and ignores non-image content', async () => {
    ...
    const result = await model.doGenerate(
      callOptions({
        prompt: 'add a hat',
        files: [{ type: 'file', mediaType: 'image/png', data: 'INPUT' }],
      }),
    );

...
describe('AihubmixFluxTransport', () => {
  ...
  it('extracts taskId from { output: [{ taskId }] } and sends prompt/aspect_ratio/seed/safety_tolerance', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ output: [{ taskId: 'task-1' }] }), { status: 200 }),
      );
  ...
  it('falls back to output[0].id when taskId is absent', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ output: [{ id: 'task-id-fallback' }] }), { status: 200 }),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });
  ...
  it('polls Pending → Ready and returns [result.sample]', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ status: 'Ready', result: { sample: 'https://img/a.png' } }), {
          status: 200,
        }),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });
  ...
  it('returns the result.samples array when sample is absent', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          status: 'Ready',
          result: { samples: ['https://img/a.png', 'https://img/b.png'] },
        }),
        {
          status: 200,
        },
  ...
  it('throws immediately on a terminal Error status without retrying', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ status: 'Error', detail: 'render failed' }), { status: 200 }),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });
  ...
  it('throws immediately on a Content Moderated status without retrying', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ status: 'Content Moderated' }), { status: 200 }),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });
  ...
  it('throws REMOTE_ERROR when Ready arrives without a sample URL', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ status: 'Ready', result: {} }), { status: 200 }),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });
  ...
  it('retries a transient non-ok poll response and then succeeds on Ready', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ error: { message: 'upstream 500' } }), { status: 500 }),
      )
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ status: 'Ready', result: { sample: 'https://img/ok.png' } }),
          { status: 200 },
        ),
      );
    const transport = createAihubmixFluxTransport({ ...settings, fetch: fetchMock as never });

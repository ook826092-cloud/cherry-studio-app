...
export const KNOWLEDGE_BASE_ERROR_MISSING_EMBEDDING_MODEL: KnowledgeBaseErrorCode =
  'missing_embedding_model';
export const KNOWLEDGE_BASE_ERROR_MISSING_VECTOR_STORE: KnowledgeBaseErrorCode =
  'missing_vector_store';
 ...
 */
export const KNOWLEDGE_ITEM_ERROR_CODES = [
  'directory_not_migrated',
...
export const KNOWLEDGE_ITEM_ERROR_DIRECTORY_NOT_MIGRATED: KnowledgeItemErrorCode =
  'directory_not_migrated';
export const KNOWLEDGE_ITEM_ERROR_INDEXING_INTERRUPTED: KnowledgeItemErrorCode =
  'indexing_interrupted';
...
const KnowledgeItemSharedSchema = z.strictObject({
  source: z
    ...
    .trim()
    .min(1)
    .describe('Original user-facing source identifier for the knowledge item.'),
});
...
const PreparingKnowledgeItemLifecycleSchema = {
  status: z
    .literal('preparing')
    .describe('Container expansion is running; only directory items use it.'),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const ProcessingKnowledgeItemLifecycleSchema = {
  status: z
    .literal('processing')
    .describe('Work has been queued or is running before a more specific phase is known.'),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const ReadingKnowledgeItemLifecycleSchema = {
  status: z
    .literal('reading')
    .describe('Leaf source documents are being read; only file, url, and note items use it.'),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const EmbeddingKnowledgeItemLifecycleSchema = {
  status: z
    .literal('embedding')
    .describe(
      'Leaf chunks are being embedded and written to the vector store; only file, url, and note items use it.',
    ),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const CompletedKnowledgeItemLifecycleSchema = {
  status: z
    .literal('completed')
    .describe('Indexing or container reconciliation finished successfully.'),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const DeletingKnowledgeItemLifecycleSchema = {
  status: z
    .literal('deleting')
    .describe('Delete cleanup is in progress; default list, search, and RAG reads hide it.'),
  error: z.null().describe('No error is stored for non-failed lifecycle states.'),
} as const;
...
const createLeafKnowledgeItemEntitySchemas = <
...
const createContainerKnowledgeItemEntitySchemas = <
  TType extends KnowledgeItemType,
  TData extends z.ZodType,
>(
  type: TType,
...
const refineRuntimeConfig = (
  value: z.infer<typeof KnowledgeBaseRuntimeConfigSchema>,
  ctx: z.RefinementCtx,
): void => {
...
const RuntimeFileItemDataSchema = KnowledgeItemSharedSchema.extend({
  path: AbsoluteFilePathSchema.describe(
...
export function getKnowledgeItemDisplayTitle(item: KnowledgeItemTitleSource): string {
  ...
  switch (item.type) {
    ...
    case 'directory':
      return getKnowledgePathBasename(data.relativePath || data.source || '');
    case 'note': {
      const snapshotName = data.relativePath
        ? getKnowledgePathBasename(data.relativePath).replace(/\.md$/i, '')
        : '';

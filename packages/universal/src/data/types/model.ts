...
const CommonReasoningFieldsSchema = {
  ...
  /** Mobile compatibility alias retained while callers move to selectableEfforts. */
  ...
  .superRefine((pricing, ctx) => {
    ...
    const rates = [
      ...
      ...(pricing.inputTokenTiers ?? []).flatMap((tier, index) => [
        ...
        ...(tier.cacheRead
          ? [{ rate: tier.cacheRead, path: ['inputTokenTiers', index, 'cacheRead'] }]
          : []),
        ...(tier.cacheWrite
          ? [{ rate: tier.cacheWrite, path: ['inputTokenTiers', index, 'cacheWrite'] }]
          : []),
      ]),
    ];
...
export const ModelSchema = z.object({
  ...
  /** Provider ID */
  providerId: z.string(),
  /** Raw provider model id retained for mobile call sites. */
  modelId: z.string(),
  ...
  customEndpointUrl: z.string().optional(),
